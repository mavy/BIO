...
if len(sys.argv) >= 7:
    ...
    print "Uso : <fenotypeData> <dataDir> <inputFile> <GWASFile>"

    sys.exit(-1)

# Paso 2b: De cada ssnp, mirar si el individuo lo tiene o no activado
# Paso 3: Sumar cada rs + 1 por activación | Separados por Pob
def contar_otros_snps(fenotypeList, all_fenotypes, simultaneous, listInd, listIndY):
                ...
                if record[2] == fenotype['SNPS']:
                    ...
                    if chromosome == 'Y':
                        ...
                        for index in humans.values():
                            if list[index]:
                                results[fenotype['SNPS']] += 1
            print len(results), '/', len(all_fenotypes)
    return results
